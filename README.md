...
`remove-list-item` removes all matching scalar values from a supported list. `append-list-item` avoids duplicates by default and only appends duplicate values when `--allow-duplicate` is supplied.

Schema normalization inserts required list fields as `[]`, required map fields as `{}` or block maps with required child placeholders, and required scalar fields with conservative defaults. It refuses source-preserving edits that would require unsafe broad rewrites.
